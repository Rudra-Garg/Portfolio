...
export function Navbar() {
    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50">
            {/* Backdrop blur */}
            <div className="absolute inset-0 bg-[#0B0F14]/80 backdrop-blur-md border-b border-slate-800/50" />
            ...
            <nav className="relative max-w-7xl mx-auto px-6">
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="hidden md:flex items-center gap-1">
                        {navigation.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className="flex items-center gap-2 px-4 py-2 text-sm text-slate-400 hover:text-cyan-400 rounded-lg hover:bg-slate-800/50 transition-all"
                            >
                    ...
                    <div className="hidden md:flex items-center gap-2">
                        {socials.map((item) => (
                            <a
                                ...
                                className="p-2 text-slate-500 hover:text-cyan-400 rounded-lg hover:bg-slate-800/50 transition-all"
                    ...
                    {/* Mobile Menu Button */}
                    <button
                        className="md:hidden p-2 text-slate-400 hover:text-cyan-400 rounded-lg hover:bg-slate-800/50 transition-all"
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="md:hidden relative bg-[#0B0F14]/95 backdrop-blur-md border-b border-slate-800/50"
                    >
                        <div className="px-6 py-4 space-y-2">
                            {navigation.map((item) => (
                                <Link
                                    key={item.name}
                                    href={item.href}
                                    className="flex items-center gap-3 px-4 py-3 text-slate-300 hover:text-cyan-400 rounded-lg hover:bg-slate-800/50 transition-all"
